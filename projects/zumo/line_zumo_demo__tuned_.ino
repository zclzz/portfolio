...
PIDLoop headingLoop(5000, 0, 2000, false);
...
void loop()
{
  ...
  // If error or nothing detected, turn right
  if (res<=0) 
  {
    right += ZUMO_FAST;
    left -= ZUMO_FAST;
...
//      }
      ...
      {
        if (pixy.line.intersections->m_n == 3) //slow down if 2 intersections
        {
          left += ZUMO_TURN;
          right -= ZUMO_TURN;
        }
